...
- ALB : 일반 웹 서비스 → 거의 다 이거 씀
- NLB : 빠르거나 특수한 통신이 필요할 때
- GWLB : 보안 전문 회사가 쓰는 특수 장비
...
- OSI 7계층 중 응용 계층을 사용해서 부하 분산 수행
- HTTP / HTTPS의 트래칙을 라우팅

#### 2. NLB (Network Load Balancer)
<img src="./image/nlb_explained.svg" width="80%" alt="type">

- **비유: 고속도로 톨게이트**
  - 차가 어디 가는지 안 물어봄 (목적지 안 봄)
  - 그냥 빨리 통과시키는 게 목적
  - 처리 속도가 압도적으로 빠름

#### 3. GWLB (Gateway Load Balancer)
<img src="./image/gwlb_explained.svg" width="80%" alt="type">

- **비유: 공항 보안 검색대**
  - 일반 웹 서비스에서는 거의 안 씀
  - 회사로 들어오는 모든 트래칙을 보안 장비에 통과시키는 역할


### 로드 밸런서의 대상 그룹과 리스너
#### 대상 그룹
- 로드 밸런서 노드와 EC2 인스턴스 사이의 중간 다리 역할 수행
- 부하 분산을 시도할 EC2 인스턴스를 등록하고 관리

#### 리스너
- n개의 대상 그룹을 운영할 때 어떠한 방식으로 접근을 할지 설정할 수 있는 기능
- 리스너 규칙을 설정하여 프로토콜과 포트를 지정하고 대상 그룹을 연결지을 수 있음
  - 포트 기반 라우팅 : 특정 프로토콜과 포트를 지정하고 대상 그룹을 연결
  - 경로 기반 라우팅 : 경로를 기반으로 특정 서버로 라우팅 하는 방법
...
### 로드 밸런서의 알고리즘
#### 라운드 로빈 방식 
- 요청을 로드밸런싱 대상 서버에 순서대로 할당받는 방식

#### 가중치 랜덤 방식
- 부하 분산 알고리즘과 함께 사용되어, 비정상적인 대상을 식별하고 트래칙을 삭제하고 오류가 발생하지 않은 대상에 자동으로 트래픽 전달
